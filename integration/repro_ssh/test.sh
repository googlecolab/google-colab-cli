...
#
# The interactive shell (raw TTY, `cd /content`) still cannot be driven
# headlessly; those remain documented manual steps at the bottom.
...
echo ""
echo "== manual-only (needs a real TTY; cannot run headlessly) =="
echo "  colab ssh -s $SESSION_NAME    # interactive shell; lands in /content,"
echo "                                # whoami -> root. 429 on a 2nd connection."
exit 0
