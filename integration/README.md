...
| `repro_run_command/` | `colab run <script.py>` runs a local script on a fresh VM and cleans up; `--keep` leaves the session alive. |
...
## Running
```bash
uv run bash integration/repro_run_command/test.sh
```
