...
def _is_scope_error(e: Exception) -> bool:
    ...
    The Boq frontend returns a `google.rpc.Status` with `code=7` (PERMISSION_DENIED)
...
def _scope_remediation_message(provider) -> str:
    ...
    if provider == AuthProvider.ADC:
        return (
            ...
            "TFE backend at colab.research.google.com) and colaboratory "
            "(required by the Boq RuntimeService at colab.pa.googleapis.com):\n"
    ...
    # LOAS2 path — should be transparent after the auth.py fix; if a stale
    # token is cached anywhere, suggest a refresh.
    return (
        f"{common}\n"
        "If you're on the corp LOAS2 provider, refresh credentials with "
        "`gcert` and re-run `colab new`. If the failure persists, the "
        "corplogin policy may not permit the colaboratory scope for your "
        "account — file an issue."
    )
...
def new(
    ...
    # Keep-alive requires the `colaboratory` OAuth scope, which the Boq
    # RuntimeService at colab.pa.googleapis.com enforces. The corp LOAS2
    # exchange policy does NOT allowlist this scope (see auth.py), so under
    # `--auth=loas2` we cannot make keep-alive succeed. We honestly disable
    # the daemon in that case rather than spawn one that's guaranteed to die
    # in ~1 minute. Tracked: go/enterprise-identity-intake (policy exception).
    if state.auth_provider == AuthProvider.LOAS2:
        typer.echo(
            "[colab] Note: keep-alive is disabled under --auth=loas2 (the corp "
            "LOAS-to-OAuth policy does not permit the 'colaboratory' scope). "
            "The VM will idle-timeout in ~90 minutes. Use --auth=oauth2 for a "
            "long-running session, or track go/enterprise-identity-intake.",
            err=True,
        )
    else:
...
def spawn_keep_alive(
    ...
    Without this, the child inherits Typer's defaults (`--auth=loas2`,
    `--config=~/.config/colab-cli/sessions.json`), which causes:
      (a) wrong auth backend (broke live ADC test 2026-04-30), and
      (b) the daemon's `state.store.get(session_name)` check finds nothing
          ...
          `--config` to write to a non-default path (broke the soak
          integration test 2026-04-30).
    """
