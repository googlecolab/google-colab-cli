...
# Internal 1P Scopes.
#
# Only `userinfo.email` can be obtained via the corp LOAS-to-OAuth path
# (stubby blade:sso CorpLogin.Exchange). The `colaboratory` scope is NOT
# allowlisted in the corplogin policy (security/corplogin/exchange/policy/
# policy.ncl), so requesting it here triggers
# "Your request did not match any of the authorized CXF policy rules".
#
# This means LOAS2 tokens cannot talk to the Boq RuntimeService at
# colab.pa.googleapis.com (which requires the colaboratory scope, returning
# 403 SCOPE_NOT_PERMITTED otherwise). In particular, `KeepAliveAssignment`
# does not work under LOAS2 — the keep-alive daemon is therefore disabled
# for this provider in commands/session.py. See go/enterprise-identity-intake
# for the open policy-exception request to lift this restriction.
...
def get_credentials(
    ...
    provider: AuthProvider = AuthProvider.LOAS2,
    ...
    """
    if provider == AuthProvider.LOAS2:
